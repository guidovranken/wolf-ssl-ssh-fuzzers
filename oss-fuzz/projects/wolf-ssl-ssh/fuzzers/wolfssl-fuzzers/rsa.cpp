    ...
    try {
        ...
        switch ( op ) {
            case    0:
                {
                    ...
                    CF_CHECK_GTE(wc_RsaPublicEncrypt_ex(
                                in.data(), in.size(),
                                out, outSize,
                                &key,
                                &rng,
                                padType,
                                static_cast<enum wc_HashType>(hashType),
                                static_cast<int>(mgf),
                                label.data(), label.size()), 0);
                }
                break;
            case    1:
                CF_CHECK_GTE(wc_RsaPSS_Sign(
                            in.data(), in.size(),
                            out, outSize,
                            static_cast<enum wc_HashType>(hashType),
                            static_cast<int>(mgf),
            ...
            case    2:
                ...
                break;
            case    3:
                CF_CHECK_GTE(wc_RsaSSL_Verify(
                            in.data(), in.size(),
                            out, outSize,
                            &key), 0);
                break;
            case    4:
                CF_CHECK_GTE(wc_RsaPSS_Verify(
                            in.data(), in.size(),
                            out, outSize,
                            static_cast<enum wc_HashType>(hashType),
                            static_cast<int>(mgf),
            ...
            case    5:
                {
                    ...
                    CF_CHECK_EQ(wc_RsaPSS_CheckPadding(
                            in.data(), in.size(),
                            sig.data(), sig.size(),
                            static_cast<enum wc_HashType>(hashType)), 0);
                }
                break;
            case    6:
                {
                    auto sig = ds.GetData(0);
                    CF_CHECK_GTE(wc_RsaPSS_VerifyCheck(
                                in.data(), in.size(),
                                out, outSize,
                                sig.data(), sig.size(),
                                static_cast<enum wc_HashType>(hashType),
